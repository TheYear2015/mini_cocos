...
class CCPluginLuaCompile(cocos.CCPlugin):
    ...
    def init(self, options, workingdir):
        ...
        self._script_test = options.script_test
    ...
    def deep_iterate_dir(self, rootDir):
        for lists in os.listdir(rootDir):
            if (not self._script_test) and len(lists) > 3:
                d = lists[-4:]
                if d.lower() == 'test':
                    continue
    ...
    def parse_args(self, argv):
        ...
        parser.add_option("--test",
                          action="store_true", dest="script_test", default=False,
                          help="Whether or not to test")
