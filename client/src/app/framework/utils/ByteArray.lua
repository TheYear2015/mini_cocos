...
function ByteArray:getBytes(__offset, __length)
    ...
    --table.foreach(self._buf,function(i, v) print (i, v) end)
...
function ByteArray:readChar()
	...
	if __val > 127 then __val = __val - 256 end
	return __val
end
...
function ByteArray:readBuf(__len)
	--print(string.format("readBuf,len:%d, pos:%d, buflen:%d", __len, self._pos, #self._buf))
